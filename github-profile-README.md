# Luis Javier Lozoya

**Full Stack Engineer | Security, Cloud, AI**

Building production applications with React, Next.js, AWS, and Python. GIAC GFACT + GSEC certified. Currently training through the SANS Cyber Academy with a focus on security-focused engineering.

## What I'm Working On

- Security lab writeups with real packet captures and analysis - [View Labs](https://www.luislozoya.com/#security-labs)
- AI-powered portfolio with fit assessment and chat - [www.luislozoya.com](https://www.luislozoya.com)
- Preparing for GCIH, AWS Security, and PWPA certifications

## Stack

**Languages:** TypeScript, JavaScript, Python, SQL
**Frontend:** React, Next.js, Svelte, Tailwind CSS
**Backend:** Node.js, Express, FastAPI
**Cloud:** AWS (EC2, S3, Lambda, CloudFront, Route 53), Vercel
**Data:** PostgreSQL, Supabase, Redis
**Security:** Wireshark, tcpdump, PCAP analysis, Network Forensics
**AI/ML:** OpenAI, LangChain, Anthropic Claude

## Certifications

- GIAC Security Essentials (GSEC)
- GIAC Foundational Cybersecurity Technologies (GFACT)
- Coursera Full Stack Development
- TryHackMe Cybersecurity Path

## Latest Blog Posts

- [Analyzing Network Traffic with tcpdump: Detecting Brute Force Attacks](https://www.luislozoya.com/blog/tcpdump-traffic-analysis)
- [Wireshark Packet Analysis: Investigating a 628K-Packet Capture](https://www.luislozoya.com/blog/wireshark-packet-analysis)

## Get in Touch

- [Portfolio](https://www.luislozoya.com)
- [LinkedIn](https://www.linkedin.com/in/luisjlozoya)
- [Blog](https://www.luislozoya.com/blog)
